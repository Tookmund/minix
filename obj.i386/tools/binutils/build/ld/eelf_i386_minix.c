...
/* 32 bit ELF emulation code for elf_i386_minix
...
#define TARGET_IS_elf_i386_minix
...
static void gldelf_i386_minix_before_parse (void);
static void gldelf_i386_minix_after_open (void);
static void gldelf_i386_minix_before_allocation (void);
static void gldelf_i386_minix_after_allocation (void);
static lang_output_section_statement_type *gldelf_i386_minix_place_orphan
  (asection *, const char *, int);

static void
gldelf_i386_minix_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldelf_i386_minix_before_parse (void)
{
...
static bfd_boolean
gldelf_i386_minix_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf_i386_minix_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf_i386_minix_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf_i386_minix_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf_i386_minix_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf_i386_minix_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf_i386_minix_search_needed (const char *path,
				    struct dt_needed *n, int force, int prepend_sysroot)
{
  ...
  if (name[0] == '/') {
    if (prepend_sysroot && ld_sysroot) {
      ...
      rv = gldelf_i386_minix_try_needed (&needed, force);
      free(filename);
      return rv;
    }
    return gldelf_i386_minix_try_needed (n, force);
  }
  ...
  while (1)
      ...
      else
	{
      ...
      needed.name = filename;
      if (gldelf_i386_minix_try_needed (&needed, force))
	return TRUE;
...
static char *
gldelf_i386_minix_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
static void
gldelf_i386_minix_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf_i386_minix_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf_i386_minix_write_build_id_section (bfd *abfd)
{
...
static void
gldelf_i386_minix_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf_i386_minix_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf_i386_minix_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf_i386_minix_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf_i386_minix_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf_i386_minix_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf_i386_minix_search_needed (command_line.rpath_link,
						  &n, force, 0))
	    break;
	  if (gldelf_i386_minix_search_needed (command_line.rpath,
						  &n, force, 1))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      found = (rp->by == l->by
		       && gldelf_i386_minix_search_needed (rp->name,
							      &n,
							      force, 1));
	    }
	  if (found)
	  ...
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf_i386_minix_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf_i386_minix_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf_i386_minix_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf_i386_minix_find_exp_assignment (exp->binary.lhs);
      gldelf_i386_minix_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf_i386_minix_find_exp_assignment (exp->trinary.cond);
      gldelf_i386_minix_find_exp_assignment (exp->trinary.lhs);
      gldelf_i386_minix_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf_i386_minix_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf_i386_minix_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf_i386_minix_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldelf_i386_minix_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldelf_i386_minix_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  lang_for_each_statement (gldelf_i386_minix_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf_i386_minix_append_to_separated_string (&depaudit, cp);
  ...
  if (sinterp != NULL)
    {
      sinterp->contents = (unsigned char *) "/libexec/ld-elf.so.1";
...
static bfd_boolean
gldelf_i386_minix_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf_i386_minix_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static void
gldelf_i386_minix_after_allocation (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);
  gldelf_i386_minix_map_segments (need_layout);
}

static char *
gldelf_i386_minix_get_script (int *isfile)
{
  *isfile = 0;
...
static bfd_boolean
gldelf_i386_minix_handle_option (int optc)
{
  switch (optc)
    {
    default:
    ...
    case OPTION_AUDIT:
	gldelf_i386_minix_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf_i386_minix_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldelf_i386_minix_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf_i386_minix_emulation =
{
  gldelf_i386_minix_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldelf_i386_minix_after_open,
  gldelf_i386_minix_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  gldelf_i386_minix_before_allocation,
  gldelf_i386_minix_get_script,
  "elf_i386_minix",
  "elf32-i386-minix",
  finish_default,
  NULL,
  gldelf_i386_minix_open_dynamic_archive,
  gldelf_i386_minix_place_orphan,
  NULL,
  NULL,
  gldelf_i386_minix_add_options,
  gldelf_i386_minix_handle_option,
  NULL,
  gldelf_i386_minix_list_options,
  gldelf_i386_minix_load_symbols,
  NULL,
  NULL
};
