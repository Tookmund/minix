				    ...
				    struct dt_needed *n, int force, int prepend_sysroot)
{
  ...
  if (name[0] == '/') {
    if (prepend_sysroot && ld_sysroot) {
      ...
      rv = gldelf_i386_try_needed (&needed, force);
      free(filename);
      return rv;
    }
...
gldelf_i386_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (link_info.relocatable && config.build_constructors)
    return "ldscripts/elf_i386.xu";
  else if (link_info.relocatable)
    return "ldscripts/elf_i386.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf_i386.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf_i386.xn";
  else if (link_info.pie && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    return "ldscripts/elf_i386.xdw";
  else if (link_info.pie && link_info.combreloc)
    return "ldscripts/elf_i386.xdc";
  else if (link_info.pie)
    return "ldscripts/elf_i386.xd";
  else if (link_info.shared && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    return "ldscripts/elf_i386.xsw";
  else if (link_info.shared && link_info.combreloc)
    return "ldscripts/elf_i386.xsc";
  else if (link_info.shared)
    return "ldscripts/elf_i386.xs";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    return "ldscripts/elf_i386.xw";
  else if (link_info.combreloc)
    return "ldscripts/elf_i386.xc";
  else
    return "ldscripts/elf_i386.x";
}
